<template>
  <v-card>
    ...
    <v-divider></v-divider>
    <v-card-text
      >Aligned {{ info.name }} {{ title }} in txt format.</v-card-text
    >
    <v-card-text class="py-0"
      >One <b>{{ lineItemName }}</b> per line.</v-card-text
    >
    <v-divider class="mt-10"></v-divider>
    <v-card-actions>
      <v-btn
        @click="downloadFile(info.langCode)"
        :loading="isLoading.download[info.langCode]"
