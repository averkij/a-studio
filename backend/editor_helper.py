...
def switch_excluded_splitted_to(db_path, id):
    ...
    with sqlite3.connect(db_path) as db:
        ...
        if exclude:
            db.execute(
                "update splitted_to set exclude=:exclude where id=:id",
...
def switch_excluded_splitted_from(db_path, id):
    ...
    with sqlite3.connect(db_path) as db:
        ...
        if exclude:
            db.execute(
                "update splitted_from set exclude=:exclude where id=:id",
                {"exclude": (exclude[0] + 1) % 2, "id": id},
            )
    return
...
def get_processing_text(db_path, text_type, processing_id):
    ...
    with sqlite3.connect(db_path) as db:
        if text_type == con.TYPE_FROM:
            cur = db.execute(
                "select text from processing_from where id = :id", {"id": processing_id}
            )
        else:
            cur = db.execute(
                "select text from processing_to where id = :id", {"id": processing_id}
            )
        res = cur.fetchone()
    return res
