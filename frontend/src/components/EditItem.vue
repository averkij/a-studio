    ...
    <v-row justify="center" class="edit-row" no-gutters>
      ...
      <v-col class="text-left" cols="6">
        <div class="d-table fill-height cell-edit">
          <!-- left line id column -->
          <div
            class="d-table-cell lighten-5 cell-edit-index text-center"
            :class="[
              { red: panelColor == 'red' },
              { green: panelColor == 'green' },
          ...
          <div
            class="d-table-cell fill-width color-transition"
            ...
            <div class="pa-2 pb-8">
              <div class="d-table fill-height fill-width">
                ...
                <div class="d-table-cell" style="width: 15px">
                  <i
                    class="v-icon mdi mdi-chevron-down theme--light"
                    style="
                      border-radius: 50%;
                      background: #f2f2f2;
                      cursor: pointer;
                    "
                    @click="toggleShowLines('from')"
                    :class="{ 'icon-avtive': showLinesFrom }"
                  ></i>
                </div>
              </div>
            </div>
            ...
            <div v-show="showLinesFrom">
              <div class="candidates-cont" v-for="(t, i) in transFrom" :key="i">
                ...
                <div class="d-table fill-height fill-width">
                    ...
                    >
                      ...
                      <div class="cell-edit-action-panel">
                        <div
                          ...
                          @click="editAddCandidateEnd('from', t.id, t.text)"
                        >
                          <v-icon>mdi-arrow-up-bold-circle</v-icon>
                        </div>
                      </div>
                  ...
                  <div
                    class="d-table-cell yellow pa-2 fill-width candidate-text"
                    :class="[
                      { 'lighten-4': isLineIdFromSelected(t.id) },
                      { 'lighten-5': !isLineIdFromSelected(t.id) },
                    ]"
                  >
                    {{ t.text }}
                    <div
                ...
                <div class="d-table-cell" style="width: 15px">
                  <i
                    ...
                    style="
                      ...
                      cursor: pointer;
                    "
                    @click="toggleShowLines('to')"
                    :class="{ 'icon-avtive': showLines }"
                  ></i>
                </div>
              </div>
            </div>
              ...
              <div class="candidates-cont" v-for="(t, i) in trans" :key="i">
                ...
                <div class="d-table fill-height fill-width">
                  ...
                  >
                    ...
                    >
                      ...
                      <div class="cell-edit-action-panel">
                        <div
                          class="cell-edit-button"
                          @click="editAddCandidateEnd('to', t.id, t.text)"
                        >
                          <v-icon>mdi-arrow-up-bold-circle</v-icon>
                        </div>
                      </div>
                  ...
                  <div
                    class="d-table-cell yellow pa-2 fill-width candidate-text"
                    :class="[
                      { 'lighten-4': isLineIdToSelected(t.id) },
                      { 'lighten-5': !isLineIdToSelected(t.id) },
                    ]"
                  >
                    {{ t.text }}
                    <div
...
export default {
  ...
  methods: {
    ...
    editAddUpEnd(textType) {
      if (textType == "from") {
        let newText = this.editedFromText
          ? this.editedFromText
          : this.item.text_from;
        this.$emit(
          "editAddUpEnd",
    ...
    getProxyFromTexts() {
      let res = JSON.parse(this.item.line_id_from)
        .map(function (num) {
    ...
    getProxyToTexts() {
      let res = JSON.parse(this.item.line_id_to)
        .map(function (num) {
